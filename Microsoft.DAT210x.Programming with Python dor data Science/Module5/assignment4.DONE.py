...
#
# .. your code here ..
df = pd.read_csv('Datasets/Wholesale customers data.csv',header=0,delimiter=',')
df.head()
...
def plot_cols(df,props = ['mean','min','max']):
    desc = df.describe()
    for col in df.columns:
        fig = plt.figure()
        ax = fig.add_subplot(111)
        df[[col]].plot.hist(ax=ax,bins=30,title=col)
        for prop in props:
            plt.axvline(desc.loc[prop,col],c='black',ls='--',alpha=0.8)
            plt.text(desc.loc[prop,col],-5,prop,rotation=45)
            plt.text(desc.loc[prop,col],ax.get_ylim()[1]/2,int(desc.loc[prop,col]))         
    plt.show()
