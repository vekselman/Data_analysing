...
comp_train = model.transform(x_train)
comp_test = model.transform(x_test)
#
...
print np.shape(comp_test)
print np.shape(y_test)
scores ={}
models ={}
for i in range(1,21):
    knn = KNeighborsClassifier(n_neighbors = i)
    models[i] = knn.fit(comp_train,y_train)
    scores[i] = knn.score(comp_test,y_test)
...
Plot2DBoundary(comp_train, y_train, comp_test, y_test)
