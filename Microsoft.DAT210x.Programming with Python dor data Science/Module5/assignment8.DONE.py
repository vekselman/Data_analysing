...
df = pd.read_csv('Datasets/life_expectancy.csv',header=0,delimiter='\t')
...
drawLine(model,X_train,y_train,'BlackFemale train')
drawLine(model,X_test,y_test,'BlackFemale test')
df[df.Year== 2014]
#
#
...
corr = df.corr()
print corr
splt = sns.heatmap(corr, 
            xticklabels=corr.columns.values,
            yticklabels=corr.columns.values)
splt.set_yticklabels(rotation=45)
plt.show()
