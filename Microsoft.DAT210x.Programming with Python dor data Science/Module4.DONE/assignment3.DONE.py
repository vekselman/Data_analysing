...
#
# .. your code here ..
nom_f = ['classification', 'rbc', 'pc',
         'pcc', 'ba', 'htn', 'dm', 'cad', 'appet', 'pe', 'ane']# to drop
...
for col in ['pcv','wc','rc']:
    num_df[[col]] = pd.to_numeric(num_df.loc[:,col],errors='coerce')
...
print(df.loc[:,nom_f].head())
nom_df = pd.get_dummies(df.loc[:,nom_f].drop('classification',axis=1))
print(nom_df.shape)

df = brw
df = num_df
df = pd.concat([num_df,nom_df],axis=1)
